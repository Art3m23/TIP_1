...
function getPrice() {
  ...
  if (arr.length !== 0 && arr1.length !== 0 && arr2.length !== 0 && arr3.length !== 0) {
    ...
    document.querySelector('#allprice').value = sum.filter(e => isNaN(e) === false).reduce((a, b) => a + b) + ' руб';
  } else {
    courses.append(error);
  }
}
document.querySelectorAll('.courses__button')[0].addEventListener('click', getPrice);
document.querySelectorAll('.courses__button')[1].addEventListener('click', () => {
  coursesItem.forEach((el) => {
    el.checked = false;
  });
  disabled.forEach(el => {
